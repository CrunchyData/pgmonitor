...
if [ -z "$BACKREST_CONFIGS" ] && [ -z "$BACKREST_STANZAS" ]; then
    ...
    do
      echo $(echo -n "$conf|" | tr '/' '_'; pgbackrest --config=$conf --output=json --log-level-console=info info | tr -d '\n') | grep $SYSTEM_ID
      if [ $? == 0 ]; then
        break
      fi
    done
    ...
    do
      ...
      echo $(echo -n "$conf|" | tr '/' '_'; pgbackrest --output=json --log-level-console=info info | tr -d '\n') | grep $SYSTEM_ID
      if [ $? == 0 ]; then
        break
      fi
    done
